...
fs.readdir(configdir, (err, files) => {
    ...
    for (var i = 1; i <= files.length; i++){
                ...
                request({ url: apiUrl, headers: { json: true, Referer: 'discord-rustserverstatus' }, timeout: 10000 }, function (err, res, body) {
                    if (!err && res.statusCode == 200) {
                        ...
                        if (is_online == "Online") {
                            ...
                            status = `${players}/${maxplayers} to join use steam://connect/45.10.25.105:27023 `
                ...
                request({ url: apiUrl, headers: { json: true, Referer: 'discord-rustserverstatus' }, timeout: 10000 }, function (err, res, body) {
                    if (!err && res.statusCode == 200) {
                        ...
                        if (is_online == "online") {
                            ...
                            if (players>40){Servermsg="Time to Kill"}
                            if((today==3||today==5||today==6) && (hours>=18 &&hours<=23) ){EventAnnouncement="EVENT!"}
                            let status = `${players}/${maxplayers} ${Servermsg} ${EventAnnouncement} ${map}`
        ...
        const updateInterval = 1000  * 3 || 1000  * process.env.updateInterval || 1000 * config.updateInterval
